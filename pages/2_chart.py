...
def init_market_db():
    ...
    cursor = conn.cursor()
    # FIXED: Added 'timeframe' inside composite primary keys to stop data mixing
    cursor.execute("""
...
# Active Sidebar Dropdown Selection Targets
...
# ==============================================================================
# 📊 3. DYNAMIC SDK TIMEFRAME SYNC PIPELINE (HISTORICAL)
# ==============================================================================
if market_engine is not None and not st.session_state.get(state_key):
    try:
        ...
        # FIXED: Pass dynamic selected_tf parameter directly to API fetch call
                    ...
                    if chart:
                        ...
                        for i in range(len(opens)):
                            try:
                                ...
                                # FIXED: Saved data explicitly with active selected_tf label key
                                cursor.execute("""
...
try:
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    # FIXED: Read chronological data filtered strictly by selected_tf dropdown
    cursor.execute("""
    ...
    for idx, row in enumerate(rows):
        ...
        st_upper = round(((h + l) / 2.0) + (2.5 * atr_range), 2)
        st_lower = round(((h + l) / 2.0) - (2.5 * atr_range), 2)
