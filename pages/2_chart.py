...
# ==============================================================================
# 🎯 1. PURE HTML/JS CO-ORDINATE TIMEOUT SYNC
# ==============================================================================
st.set_page_config(layout="wide", page_title="SmartWealth Premium Terminal")

# 🔄 5-Second Safe Event Counter to push streaming memory updates into JavaScript Frame smoothly
...
# Master Storage Memory Allocation Guard (Prevents list index mismatch crashing)
if "master_storage" in st.session_state:
    if not isinstance(st.session_state.master_storage, dict):
...
# ==============================================================================
# 🎯 2. SIDEBAR CONFIGURATION (Includes 1 Day Timeframe Map Fix)
# ==============================================================================
...
# ✅ TIMEFRAME FIX MAP: '1 Day' explicitly locked into the structural socket engine contract
timeframe_mapping = {
    "1 Minute": "1m",
    "5 Minutes": "5m",
    "10 Minutes": "10m",
...
# Initialize internal timeframe dictionary block if missing
...
def initialize_live_ohlcv_stream():
    try:
        ...
        def on_ohlcv_data(msg):
            try:
                ...
                if sym in ["NIFTY", "SENSEX", "HDFCBANK"]:
                    # Converting integer paise to clean decimal rupee float mapping
                    ...
                    if c > 0:
                        # ✅ Strict V3 Time Stamp format parsing logic for 1 Day interval
                        if msg_tf == "1d":
                        ...
                        # Writing dynamic elements securely into active state memory buffer
        ...
        socket = websocketdata.NubraDataSocket(
            ...
            on_connect=lambda m: print("[status] WebSocket Client Fully Activated"),
            on_close=lambda r: print(f"Closed: {r}"),
            on_error=lambda e: print(f"Error: {e}")
        )
        socket.connect()
        
        # Registering subscriptions including explicit '1d' time frame token
...
if len(buf_slice) == 0:
    ...
    # Pre-populating historical depth array context to prevent frozen dashboard look
    for i in range(45):
        if interval == "1d":
...
# ==============================================================================
# Slicing structural JSON object to map only active asset's timeline down to JavaScript window variables
active_chart_data = {
    target_symbol: {
        "price": cell["price"],
        "status": cell["status"],
...
status_label = f"📁 OFFLINE BACKUP LOCK: {selected_backup_file}" if is_backup_loaded_flag else f"⚡ {target_symbol} REAL-TIME LIVE LINK ACTIVE"

st.markdown(f"""
<div class="tc-dashboard-header">
    <div class="tc-title">📊 {status_label} ({selected_tf_label})</div>
    <div class="tc-metrics-container">
        ...
        <span class="tc-badge badge-pp">⚖️ MID-PIVOT (PP): {p_point}</span>
    </div>
</div>
...
if os.path.exists(html_file_path):
    ...
    # Dynamic cross domain script bridging parameters
