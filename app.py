...
# ================= 5. S/R & SIGNALS (ORIGINAL MANUAL ENTRY) =================
st.markdown("---")
s_cols = st.columns(5)
if is_admin:
    r2 = s_cols[0].text_input("R2", value=data["sr"].get("r2", "-"))
    r1 = s_cols[1].text_input("R1", value=data["sr"].get("r1", "-"))
    s1 = s_cols[2].text_input("S1", value=data["sr"].get("s1", "-"))
    s2 = s_cols[3].text_input("S2", value=data["sr"].get("s2", "-"))
    if s_cols[4].button("Update SR"):
        data["sr"] = {"s1": s1, "s2": s2, "r1": r1, "r2": r2}
        save_json(DATA_FILE, data)
        st.rerun()
else:
    s_cols[0].error(f"R2: {data['sr'].get('r2')}")
    s_cols[1].error(f"R1: {data['sr'].get('r1')}")
    s_cols[2].success(f"S1: {data['sr'].get('s1')}")
    s_cols[3].success(f"S2: {data['sr'].get('s2')}")

st.markdown("---")
sig_cols = st.columns(5)
if is_admin:
    strike = sig_cols[0].text_input("Strike", value=data["signal"].get("Strike", "-"))
    entry = sig_cols[1].text_input("Entry", value=data["signal"].get("Entry", "-"))
    target = sig_cols[2].text_input("Target", value=data["signal"].get("Target", "-"))
    sl = sig_cols[3].text_input("SL", value=data["signal"].get("SL", "-"))
    if sig_cols[4].button("Update Signal"):
        data["signal"] = {"Strike": strike, "Entry": entry, "Target": target, "SL": sl, "Status": "LIVE"}
        save_json(DATA_FILE, data)
        st.rerun()
else:
    sig_cols[0].info(f"STRIKE: {data['signal'].get('Strike')}")
    sig_cols[1].success(f"ENTRY: {data['signal'].get('Entry')}")
    sig_cols[2].warning(f"TARGET: {data['signal'].get('Target')}")
    sig_cols[3].error(f"SL: {data['signal'].get('SL')}")

# ================= 6. OPTION CHAIN (ORIGINAL TABLE & COLOR) =================
st.markdown("---")
