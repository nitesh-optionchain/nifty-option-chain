...
# ==============================================================================
# 🗄️ 1. DATABASE SCHEMA INITIALIZATION (WITH TIMEFRAME KEY)
# ==============================================================================
def init_market_db():
...
# ==============================================================================
# 📊 3. HISTORICAL SYNC PIPELINE (REMOVED SESSION_STATE BLOCK LOCKS)
# ==============================================================================
def sync_historical_candles(asset_name, engine, timeframe):
    if engine is None:
        return
    try:
        # Check if we already have sufficient local data for this timeframe to avoid API spamming
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) FROM market_history WHERE asset=? AND timeframe=?", (asset_name, timeframe))
        count = cursor.fetchone()[0]
        
        # If database is dry or has very few bars, force fetch from broker API
        if count < 50:
...
# Always invoke clean fetch validation without session locks freezing the workflow
sync_historical_candles(target_index, market_engine, selected_tf)

# ==============================================================================
# ⚡ 4. LIVE INTERACTIVE TICK PIPELINE
# ==============================================================================
...
# ==============================================================================
# 🧠 5. CALCULATIONS ARRAY METRIC ENGINE
# ==============================================================================
