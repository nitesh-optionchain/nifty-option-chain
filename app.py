...
if result and result.chain:
    ...
    max_oi_ce = df["open_interest_CE"].max() if df["open_interest_CE"].max() > 0 else 1
    max_oi_pe = df["open_interest_PE"].max() if df["open_interest_PE"].max() > 0 else 1
    max_vol_ce = df["volume_CE"].max() if df["volume_CE"].max() > 0 else 1
    max_vol_pe = df["volume_PE"].max() if df["volume_PE"].max() > 0 else 1
    ...
    if spot >= be_res_strike:
        st.success(f"🚀 BIG MOVE: CALL BUYING ABOVE {be_res_strike}")
    elif spot <= be_sup_strike:
        st.error(f"🩸 BIG MOVE: PE BUYING BELOW {be_sup_strike}")

    # --- ADMIN & METRICS ---
    if st.session_state.is_super_admin:
        with st.expander(f"🛠️ ADMIN CONTROLS"):
    ...
    def style_table(row):
        s = [''] * len(row)
        try: strike = int(float(row.iloc[3]))
        ...
        try:
            ...
            if s[0] == '': # Only if not highlighted
            ...
            if strike == int(atm_strike): s[3] = 'background-color:yellow;color:black;font-weight:bold'
        except: pass
        return s
