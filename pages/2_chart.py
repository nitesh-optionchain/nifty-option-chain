...
# 🔒 ==============================================================================
# 🎯 1. AUTHENTICATION SYSTEM & STATE HANDSHAKE
# ==============================================================================
...
# ==============================================================================
# ⏱️ 2. STABLE AUTO-REFRESH TIMER LOOP (15 SECONDS)
# ==============================================================================
st_autorefresh(interval=15000, key="smartwealth_index_terminal_final_v1")
...
# 📂 STORAGE FILE LOADER FOR INDEX ASSETS
...
def create_fallback_data(symbol, tf_interval):
    ...
    return pd.DataFrame({"open": opens, "high": highs, "low": lows, "close": closes, "volume": np.random.randint(1000, 50000, 100)}, index=timestamps)
# ==============================================================================
# 🚀 4. CORE ENGINE DATA LOAD & PROCESSING FOR INDEX
# ==============================================================================
df = None
if st.session_state['fallback_active_state'] or market_data is None:
    ...
    try:
        with st.spinner(f"Requesting Live Dataset for {target_symbol}..."):
            ...
            if response and response.result and len(response.result) > 0:
                ...
                if target_symbol in instrument_dict:
                    ...
                    data = {"open": [float(p.value / 100.0) for p in stock_chart.open], "high": [float(p.value / 100.0) for p in stock_chart.high], "low": [float(p.value / 100.0) for p in stock_chart.low], "close": [float(p.value / 100.0) for p in stock_chart.close], "cumulative_volume": v_list}
...
# 🔥 FIXED: Rock solid SuperTrend Vector loop computation to stop layout crashes
...
df['supertrend'] = df['supertrend'].ffill().fillna(df['close'])
...
# ==============================================================================
# 🖥️ 5. PLOTLY CHART VIEWPORT NODE (HEIGHT FIXED AT 520)
# ==============================================================================
...
<div class="tc-dashboard-header">
    ...
    <div class="tc-metrics-container">
        ...
        <span class="tc-badge badge-pp">⚖ Rose Pivot: {p_point}</span>
    </div>
</div>
""", unsafe_allow_html=True)
...
fig.update_layout(height=520, xaxis_rangeslider_visible=False, template="plotly_dark", margin=dict(l=10, r=140, t=10, b=25), yaxis=dict(side="right", showgrid=True, gridcolor="#1e293b", range=[min_p * 0.99, max_p * 1.01], autorange=False, fixedrange=False), xaxis=dict(showgrid=True, gridcolor="#1e293b", autorange=True, fixedrange=False), paper_bgcolor='#030712', plot_bgcolor='#030712')
