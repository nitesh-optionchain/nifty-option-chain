...
if is_admin:
    ...
    with c5: 
        if st.button("📢 UPDATE"):
            st.session_state.signal = {"Strike": s_strike, "Entry": s_entry, "Target": s_target, "SL": s_sl, "Status": f"LIVE ({current_admin_name})"}
else:
...
def final_style(row):
    styles = [''] * len(row)
    ce_p = float(row.iloc[2].split('\n')[-1].replace('%',''))
    pe_p = float(row.iloc[4].split('\n')[-1].replace('%',''))
