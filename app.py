...
# ================= 3. LOGIN FIREWALL (SAME AS BEFORE) =================
...
if result and result.chain:
    ...
    # ================= 6. AUTO SIGNAL (CLEAN UI) =================
    if (df["oi_chg_CE"].max() / max_chg_ce * 100) > 85 and spot > be_res:
        st.error(f"🚀 BIG MOVE: CALL BREAKOUT ABOVE {be_res}")
    elif (df["oi_chg_PE"].max() / max_chg_pe * 100) > 85 and spot < be_sup:
        st.warning(f"📉 BIG MOVE: PUT BREAKDOWN BELOW {be_sup}")

    # Top Metrics (Original Look)
    ma, mb = st.columns(2)
    ma.metric("🟢 SUPPORT", current_idx_data["sr"]["support"])
    mb.metric("🔴 RESISTANCE", current_idx_data["sr"]["resistance"])

    # ================= 7. TABLE UI & STYLING (THE ORIGINAL) =================
    def fmt_val(val, delta, m_val):
        return f"{val:,.0f}\n({delta:+,})\n{(val/m_val*100 if m_val>0 else 0):.1f}%"

    def fmt_chg(delta, m_delta):
        return f"{delta:+,}\n{(delta/m_delta*100 if m_delta>0 else 0):.1f}%"
    ...
    def style_table(row):
        ...
        # Break-Even Line (Silent Upgrade)
        if strike == be_res: s = ['border: 2.5px solid #0d47a1'] * len(row)
        if strike == be_sup: s = ['border: 2.5px solid #b71c1c'] * len(row)
        ...
        try:
            ...
            if strike == df.loc[atm_idx, "STRIKE"]: s[3] = 'background-color:yellow;color:black'
        except: pass
        return s

    st.table(ui.style.apply(style_table, axis=1))

    # Admin Panel (Original Tabs Style)
    if st.session_state.is_super_admin:
        with st.expander("🛠️ ADMIN CONTROLS"):
            t1, t2 = st.tabs(["Signal", "Users"])
            with t1:
                c1, c2, c3, c4 = st.columns(4)
                s_stk = c1.text_input("Strike", current_idx_data["signal"]["Strike"])
                s_ent = c2.text_input("Entry", current_idx_data["signal"]["Entry"])
                sup_in = st.text_input("Support", current_idx_data["sr"]["support"])
                res_in = st.text_input("Resistance", current_idx_data["sr"]["resistance"])
                if st.button("UPDATE"):
                    ...
                    st.rerun()
            with t2:
                new_uid = st.text_input("Mobile ID")
                if st.button("ADD"):
                    ADMIN_DB[new_uid] = "New User"
