...
def get_engine():
    ...
    try:
        nubra = None
        # 1. Cloud Safety: Check if Streamlit Cloud secrets are active
        if "PHONE_NO" in st.secrets and "MPIN" in st.secrets:
            try:
                nubra = InitNubraSdk(NubraEnv.PROD, phone_no=st.secrets["PHONE_NO"], mpin=st.secrets["MPIN"])
            except Exception:
                nubra = None
                
        # 2. Local Safety: Fallback to PC .env file
        if nubra is None:
        ...
        socket.connect()
        socket.subscribe(["NIFTY", "SENSEX", "BANKNIFTY", "INDIAVIX"])
        
        import threading
        threading.Thread(target=socket.keep_running, daemon=True).start()
        
        # 🎯 FIXED INDENTATION: Yeh dono lines ab get_engine ke block me hain, on_msg me nahi!
