...
if result and result.chain:
    ...
    def get_signal_logic(row, side):
        stk = int(row["STRIKE"])
        # Arrow sirf breakout line ke under dikhega
        if side == "CE" and stk == be_res_strike:
            return "⬆️ BUY CE" if spot >= be_res_strike else "........."
        if side == "PE" and stk == be_sup_strike:
            return "⬇️ BUY PE" if spot <= be_sup_strike else "........."
        return "........."
    ...
    ui = pd.DataFrame()
    ui["CE SIGNAL"] = d_df.apply(lambda r: get_signal_logic(r, "CE"), axis=1) # 0
    ui["CE OI\n(Δ/%)"] = d_df.apply(lambda r: fmt_val(r["open_interest_CE"], r["oi_chg_CE"], max_oi_ce), axis=1) # 1
    ui["CE OI CHG"] = d_df.apply(lambda r: fmt_chg(r["oi_chg_CE"], max_chg_ce), axis=1) # 2
    ui["CE VOL\n(%)"] = d_df.apply(lambda r: fmt_val(r["volume_CE"], 0, max_vol_ce), axis=1) # 3
    ui["STRIKE"] = d_df["STRIKE"] # 4
    ui["PE VOL\n(%)"] = d_df.apply(lambda r: fmt_val(r["volume_PE"], 0, max_vol_pe), axis=1) # 5
    ui["PE OI CHG"] = d_df.apply(lambda r: fmt_chg(r["oi_chg_PE"], max_chg_pe), axis=1) # 6
    ui["PE OI\n(Δ/%)"] = d_df.apply(lambda r: fmt_val(r["open_interest_PE"], r["oi_chg_PE"], max_oi_pe), axis=1) # 7
    ui["PE SIGNAL"] = d_df.apply(lambda r: get_signal_logic(r, "PE"), axis=1) # 8
