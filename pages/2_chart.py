...
# ⚡ DYNAMIC HISTORICAL DATA FETCH ENGINE (Strict Documentation Alignment)
if market_engine:
    try:
        from datetime import datetime, timedelta
        end_dt = datetime.utcnow()
        # Pichle 5 din ka raw intraday data range
        start_dt = end_dt - timedelta(days=5)
        
        start_str = start_dt.strftime("%Y-%m-%dT00:00:00.000Z")
        end_str = end_dt.strftime("%Y-%m-%dT23:59:59.000Z")
        
        # Helper function to unpack Nubra timestamp points safely
        ...
        try:
            nifty_res = market_engine.historical_data({
                ...
                "fields": ["open", "high", "low", "close"], # Volume field removed as requested!
            ...
            if nifty_res and hasattr(nifty_res, 'result') and nifty_res.result and len(nifty_res.result) > 0:
                # Loop matching the official documentation structure
        ...
        # Live spot numbers for header sync remain safe
