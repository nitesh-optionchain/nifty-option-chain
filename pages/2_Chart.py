...
# 🎯 REPLACED FROM LINE 88 TO 98: DYNAMIC CLOUD RE-CONNECT BUFFER
if 'nubra_session' in st.session_state and st.session_state['nubra_session'] is not None:
    ...
    with st.spinner("Re-linking Secure Live Market Stream Server... ⏳"):
        try:
            ...
            market_data = MarketData(nubra_client)
            st.success("🔌 Secure Connection Re-established Automatically!")
...
with c3:
    st.warning(f"🟡 **Live Market LTP:** ₹{current_ltp:.2f}")import streamlit as st
import pandas as pd
import numpy as np
import plotly.graph_objects as gr
from plotly.subplots import make_subplots
