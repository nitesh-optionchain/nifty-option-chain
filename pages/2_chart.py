...
if "global_market_engine" in st.session_state and st.session_state.global_market_engine is not None:
    market_data = st.session_state.global_market_engine
else:
    try:
        client = InitNubraSdk(NubraEnv.PROD, env_creds=True)
        market_data = MarketData(client)
        st.session_state.global_market_engine = market_data
    except Exception as e:
        if 'market_engine' in st.session_state:
            market_data = st.session_state['market_engine']
    ...
    with st.spinner(f"Unpacking live structural data metrics for {target_symbol}..."):
        ...
        try:
            response = market_data.historical_data({
                ...
                "interval": interval,
                "intraDay": True if interval != "1d" else False,
                "realTime": False
            })
            ...
            if response and hasattr(response, 'result') and response.result:
                ...
                if stock_chart and hasattr(stock_chart, 'close') and stock_chart.close:
                    ...
                    v_list = [float(p.value) for p in stock_chart.cumulative_volume] if hasattr(stock_chart, 'cumulative_volume') and stock_chart.cumulative_volume else [0.0] * total_elements
                    ...
                    df['volume'] = df['cumulative_volume'].diff().clip(lower=0).fillna(0).astype(float)
                    
                    df = calculate_indicators(df, st_multiplier, st_period, rsi_period, interval)

                    chart_df = df.reset_index().copy()
                    chart_df.rename(columns={chart_df.columns[0]: "time"}, inplace=True)
