...
if result and result.chain:
    ...
    df["STRIKE"] = (df["strike_price"]/100).astype(int) if index_choice == "SENSEX" else df["strike_price"].astype(int)
    ...
    # Arrow logic with safety buffer
    def get_indicator(row, side):
        stk = int(row["STRIKE"])
        if side == "CE" and stk == be_res_strike and spot >= (stk - 5): return "⬆️ BUY CE"
        if side == "PE" and stk == be_sup_strike and spot <= (stk + 5): return "⬇️ BUY PE"
        return ""
    ...
    def style_table(row):
        ...
        if cur_strike == be_res_strike: 
            s = ['border: 3px solid blue; font-weight: bold'] * len(row)
        ...
        if cur_strike == be_sup_strike: 
            s = ['border: 3px solid red; font-weight: bold'] * len(row)
