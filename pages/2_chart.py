...
# ==============================================================================
# 🎯 1. TERMINAL CONFIGURATION & HIGH-SPEED TIMER
# ==============================================================================
st.set_page_config(layout="wide", page_title="SmartWealth Base Terminal")

# 🔄 5-Second Rapid Event Sync Counter to push stream memory state into components
...
# 📂 BACKUP SYSTEM DIRECTORY ROUTES (CSV Logs Folder)
BACKUP_DIR = "chart_backups"
if not os.path.exists(BACKUP_DIR):
    os.makedirs(BACKUP_DIR)

# Clear structural context state variants to guarantee fresh compilation execution
...
# Master Data Store Memory Frames Dictionary mapping values
...
# ==============================================================================
# 🎯 2. SIDEBAR CONTROLS LAYOUT (Includes Asset, Timeframe Selection Matrix)
# ==============================================================================
...
# ✅ TIMEFRAME FIX MAP: Fully updated to strictly capture '1d' parameters seamlessly
timeframe_mapping = {
    "1 Minute": "1m",
    "5 Minutes": "5m",
    "10 Minutes": "10m",
...
def initialize_live_ohlcv_stream():
    try:
        # Initializing target authenticated environment connection blueprint
        ...
        def capture_stream_ohlcv(msg):
            try:
                ...
                if sym in ["NIFTY", "SENSEX", "HDFCBANK"]:
                    # Precise scale divisor conversion mapping from your exact terminal print logs
                    ...
                    if c > 0:
                        # Direct timestamp logic alignment to perfectly bridge 1 Day candles
                        if msg_tf == "1d":
                        ...
                        # Append raw incoming packets seamlessly inside target timeframe arrays
        ...
        socket.connect()
        # Registering subscriptions globally across all active selectable parameters including '1d'
...
# ==============================================================================
# 🧠 4. HYBRID STABLE LOADING PIPELINE (Bypasses Static Frozen Look)
# ==============================================================================
df = None
...
if df is None:
    ...
    if len(buf) > 3:
        ...
        # Incremental backup seeder to protect initial layout grids
        mock_ticks = []
        if target_symbol == "NIFTY": base_val = 24220.0
        elif target_symbol == "SENSEX": base_val = 77410.0
        else: base_val = 1612.0
